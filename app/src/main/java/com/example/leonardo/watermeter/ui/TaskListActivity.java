...
public class TaskListActivity extends Activity {
    ...
    private List<DetailData> detailDataListAll, detailDataListNotYet, detailDataListDone, detailDataListExcepition;
        ...
        public boolean onNavigationItemSelected(@NonNull MenuItem item) {
            switch (item.getItemId()) {
                ...
                case R.id.navigation_no_task:
                    ...
                    setListViewAdapter();
                    return true;
                case R.id.navigation_done_task:
                    subDetailDataList.clear();
                    whichPage = "003";
                    updateData();
                    updateTipInfo();
    ...
    public void initView() {
        searchE = findViewById(R.id.searchE_bch);
        clearSearch = findViewById(R.id.clearSearch_bch);
        taskListView = findViewById(R.id.taskListView_bch);
        taskCountTextView = findViewById(R.id.taskCount);
        /**
         * NOTICE 下面这个list数据要一直保存，作为元数据
         * 这个列表用来作为存储用于当前显示的列表，并且当前所有的操作也是要基于这个列表的
         */
        detailDataListAll = LitePal.select("t_card_num", "t_meter_num", "t_ticket_name", "t_location", "isChecked", "t_cbyf", "t_volume_num", "t_id", "isUpload ", "t_volume_order", "t_meter_num", "sealNo").where("t_cbyf = ? and t_volume_num = ? and divideNumber = ?", cbyf, bch, divideNumber).find(DetailData.class);
        detailDataListNotYet = LitePal.select("t_card_num", "t_meter_num", "t_ticket_name", "t_location", "isChecked", "t_cbyf", "t_volume_num", "t_id", "isUpload ", "t_volume_order", "t_meter_num", "sealNo").where("t_cbyf = ? and t_volume_num = ? and divideNumber = ? and isChecked = ?", cbyf, bch, divideNumber, "1").find(DetailData.class);
        detailDataListDone = LitePal.select("t_card_num", "t_meter_num", "t_ticket_name", "t_location", "isChecked", "t_cbyf", "t_volume_num", "t_id", "isUpload ", "t_volume_order", "t_meter_num", "sealNo").where("t_cbyf = ? and t_volume_num = ? and divideNumber = ?  and isChecked = ?", cbyf, bch, divideNumber, "0").find(DetailData.class);
        detailDataListExcepition = LitePal.select("t_card_num", "t_meter_num", "t_ticket_name", "t_location", "isChecked", "t_cbyf", "t_volume_num", "t_id", "isUpload ", "t_volume_order", "t_meter_num", "sealNo").where("t_cbyf = ? and t_volume_num = ? and divideNumber = ?  and isChecked = ? and water_data_exception_status = ? ", cbyf, bch, divideNumber, "0", "1").find(DetailData.class);
    ...
    private void updateData() {
        switch (whichPage) {
            ...
            case "003":
                ...
                break;
            case "004":
                subDetailDataList = LitePal.select("t_card_num", "t_meter_num", "t_ticket_name", "t_location", "isChecked", "t_cbyf", "t_volume_num", "t_id", "isUpload ", "t_volume_order", "t_meter_num").where("t_cbyf = ? and t_volume_num = ? and divideNumber = ?  and isChecked = ? and water_data_exception_status = ? ", cbyf, bch, divideNumber, "0", "1").find(DetailData.class);
                break;
        }
    }
        ...
        try {
            baseAdapter = new BaseAdapter() {
                ...
                public View getView(int position, View convertView, ViewGroup parent) {
                    ...
                    taskIDTextView = view.findViewById(R.id.taskID);
                    cardNumberTextView = view.findViewById(R.id.cardNumberTextView);
                    meterNumberTextView = view.findViewById(R.id.meterNumberTextView);
                    ticketNameTextView = view.findViewById(R.id.ticketNameTextView);
                    locationTextView = view.findViewById(R.id.locationTextView);
                    isCheckedTextView = view.findViewById(R.id.isChecked);
        ...
        if (!searchText.isEmpty() && searchText.length() != 0) {
            ...
            switch (whichPage) {
                ...
                case "003":
                    ...
                    break;
                case "004":
                    length = detailDataListExcepition.size();
                    currentdata = detailDataListExcepition;
                    break;
            }
            for (int i = 0; i < length; i++) {
                DetailData detailDataTemp = currentdata.get(i);
                if (detailDataTemp.getT_card_num().contains(searchText) || detailDataTemp.getT_ticket_name().contains(searchText) || detailDataTemp.getT_meter_num().contains(searchText) || detailDataTemp.getSealNo().contains(searchText)) {
